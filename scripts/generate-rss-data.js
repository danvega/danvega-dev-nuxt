...
async function generateRSSData() {
  try {
    ...
    // Write RSS data to a JSON file that can be imported by the server route
    const outputPath = join(process.cwd(), '.nuxt/rss-data.json')
    await writeFile(outputPath, JSON.stringify(sortedPosts, null, 2))

    console.log(`Generated RSS data with ${sortedPosts.length} published posts`)
    console.log(`RSS data written to ${outputPath}`)
